...
+ (CJMValidationResult *)cleanMultiValuePropertyKey:(NSString *)name {
    ...
    CJMValidationResult *vr = [CJMValidator cleanObjectKey:name];
    ...
    if(name != nil && ![name isEqualToString:@""]) {
        ...
        KnownField kf = [CJMKnownProfileFields getKnownFieldIfPossibleForKey:name];
        if (kf != UNKNOWN) {
...
+ (CJMValidationResult *)cleanObjectValue:(NSObject *)o context:(CJMValidatorContext)context {
    ...
    if ([o isKindOfClass:[NSNumber class]]) {
        ...
        if (context == CJMValidatorContextProfile) {
