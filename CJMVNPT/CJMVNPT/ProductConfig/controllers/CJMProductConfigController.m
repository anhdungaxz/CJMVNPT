...
@class CJMConfigValue;

@interface CJMProductConfigController() {
...
@property (nonatomic, weak) id<CJMProductConfigDelegate> _Nullable delegate;

@end

typedef void (^CJMProductConfigOperationBlock)(void);

@implementation CJMProductConfigController
...
- (void)_updateProductConfig:(NSArray<NSDictionary*> *)productConfig isNew:(BOOL)isNew {
    CJMLogInternal(_config.logLevel, @"%@: updating product config: %@", self, productConfig);
    ...
    for (NSDictionary *kv in productConfig) {
        @try {
            ...
            CJMLogDebug(_config.logLevel, @"%@: error parsing product config key-value: %@, %@", self, kv, e.debugDescription);
            continue;
        }
    }
...
- (void)_updateActiveProductConfig:(BOOL)activated {
    CJMLogInternal(_config.logLevel, @"%@: activating product config", self);
    ...
    for (NSString *key in activeConfig) {
        @try {
            ...
            if ([value isKindOfClass:[NSData class]]) {
                ...
                CJMLogDebug(_config.logLevel, @"%@: error setting product config value: %@", self, value);
                continue;
            }
            store[key] = [[CJMConfigValue alloc] initWithData:valueData];
        } @catch (NSException *e) {
            CJMLogDebug(_config.logLevel, @"%@: error parsing product config key-value: %@, %@", self, activeConfig, e.debugDescription);
            continue;
        }
    }
...
- (void)_unarchiveDataSync:(BOOL)sync {
    ...
    __weak CJMProductConfigController *weakSelf = self;
    CJMProductConfigOperationBlock opBlock = ^{
        NSArray *data = [CJMPreferences unarchiveFromFile:filePath removeFile:YES];
        if (data) {
...
- (void)setDefaultsFromPlistFileName:(NSString *_Nullable)fileName {
    ...
    CJMLogDebug(_config.logLevel, @"%@: The plist file %@ could not be found", fileName, self);
}

- (CJMConfigValue *_Nullable)get:(NSString* _Nonnull)key {
    CJMLogInternal(_config.logLevel, @"%@: get product config for key: %@", self, key);
    @try {
        if (!key) {
            CJMLogDebug(_config.logLevel, @"%@: product config key not found", self);
            return [[CJMConfigValue alloc] initWithData:[NSData data]];
        }
        CJMConfigValue *value = self.activeConfig[key];
        if (value) {
            return value;
        } else {
            CJMLogDebug(_config.logLevel, @"%@: product config for key %@ not found", self, key);
            return [[CJMConfigValue alloc] initWithData:[NSData data]];
        }
    } @catch (NSException *e) {
        CJMLogDebug(_config.logLevel, @"%@: error parsing product config for key: %@ not found", self, key);
        return [[CJMConfigValue alloc] initWithData:[NSData data]];
    }
}
