...
+ (id)unarchiveFromFile:(NSString *)filename removeFile:(BOOL)remove {
    ...
    @try {
        ...
        CJMLogStaticInternal(@"%@ unarchived data from %@: %@", self, filePath, data);
    }
    @catch (NSException *e) {
        CJMLogStaticInternal(@"%@ failed to unarchive data from %@", self, filePath);
    }
    ...
    if (remove && [[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        ...
        if (!removed) {
            CJMLogStaticInternal(@"%@ unable to remove archived file at %@ - %@", self, filePath, error);
        }
    }
...
+ (BOOL)archiveObject:(id)object forFileName:(NSString *)filename {
    ...
    NSString *filePath = [self filePathfromFileName:filename];
    CJMLogStaticInternal(@"%@ archiving data to %@: %@", self, filePath, object);
    
    BOOL success = [NSKeyedArchiver archiveRootObject:object toFile:filePath];
    if (!success) {
        CJMLogStaticInternal(@"%@ failed to archive data to %@: %@", self, filePath, object);
    }
