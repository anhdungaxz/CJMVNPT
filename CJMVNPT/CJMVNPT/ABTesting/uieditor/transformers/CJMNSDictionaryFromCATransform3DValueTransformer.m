...
static NSDictionary *CJMCATransform3DCreateDictionaryRepresentation(CATransform3D transform)
{
    return @{
        @"m11": @(transform.m11),
        @"m12": @(transform.m12),
...
static BOOL CJMCATransform3DMakeWithDictionaryRepresentation(NSDictionary *dictionary, CATransform3D *transform)
{
    if (transform) {
...
@implementation CJMNSDictionaryFromCATransform3DValueTransformer
...
- (id)transformedValue:(id)value {
    if ([value respondsToSelector:@selector(CATransform3DValue)]) {
        return CJMCATransform3DCreateDictionaryRepresentation([value CATransform3DValue]);
    }
    return @{};
}
...
- (id)reverseTransformedValue:(id)value {
    ...
    if ([value isKindOfClass:[NSDictionary class]] && CJMCATransform3DMakeWithDictionaryRepresentation(value, &transform)) {
