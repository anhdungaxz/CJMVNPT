...
@implementation CJMNSDictionaryFromNSAttributedStringValueTransformer
...
- (id)transformedValue:(id)value {
    if ([value isKindOfClass:[NSAttributedString class]]) {
        ...
        if (data) {
            ...
            CJMLogStaticDebug(@"Failed transformation from NSAttributedString to HTML: %@", error);
        }
    }
    return nil;
}
    ...
    if ([value isKindOfClass:[NSDictionary class]]) {
        ...
        if ([mimeType isEqualToString:@"text/html"] && dataString) {
            ...
            if (attributedString == nil) {
                CJMLogStaticDebug(@"Failed transformation from HTML to NSAttributedString: %@", error);
            }
