...
@interface CJMObjectFilter: NSObject
...
@implementation CJMObjectSelector

+ (CJMObjectSelector *)objectSelectorWithPath:(NSString *)path {
    return [[CJMObjectSelector alloc] initWithPath:path];
}
...
- (NSArray *)selectFromRoot:(id)root evaluatingFinalPredicate:(BOOL)finalPredicate {
    ...
    if (root) {
        ...
        for (CJMObjectFilter *filter in _filters) {
...
- (BOOL)isLeafSelected:(id)leaf fromRoot:(id)root evaluatingFinalPredicate:(BOOL)finalPredicate {
    ...
    while (i--) {
        CJMObjectFilter *filter = _filters[i];
        filter.nameOnly = (i == n-1 && !finalPredicate);
...
- (Class)getRootViewControllerClass {
    CJMObjectFilter *filter = _filters.firstObject;
...
- (Class)selectedClass {
    CJMObjectFilter *filter = _filters.lastObject;
    if (filter) {
        return NSClassFromString(filter.name);
    }
    return nil;
...
- (BOOL)isEqual:(id)other {
    if (other == self) {
        return YES;
    } else if (![other isKindOfClass:[CJMObjectSelector class]]) {
        return NO;
    } else {
        return [self.path isEqual:((CJMObjectSelector *)other).path];
    }
}
...
@implementation CJMObjectFilter
...
- (NSArray *)getParentsOfObject:(NSObject *)obj {
    ...
    if ([obj isKindOfClass:[UIView class]]) {
        ...
        UIWindow *keyWindow = [CJMInAppResources getSharedApplication].keyWindow;
