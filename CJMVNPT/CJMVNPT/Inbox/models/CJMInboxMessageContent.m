...
@implementation CJMInboxMessageContent
...
- (instancetype)initWithJSON:(NSDictionary *)jsonObject {
    if (self = [super init]) {
        @try {
            ...
            CJMLogStaticDebug(@"Error intitializing CleverTapInboxMessageContent: %@", e.reason);
            return nil;
        }
    }
    return self;
...
- (NSString *_Nullable)urlForLinkAtIndex:(int)index {
    ...
    @try {
        ...
        CJMLogStaticInternal(@"Error getting url for link: %@", e.reason);
    }
    return url;
}
...
- (NSDictionary *_Nullable)customDataForLinkAtIndex:(int)index {
    ...
    @try {
        ...
        CJMLogStaticInternal(@"Error getting customData for link: %@", e.reason);
    }
    return customData;
}
