...
@implementation CJMInboxBaseMessageCell
...
- (void)configureForMessage:(CJMInboxMessage *)message {
    ...
    if (message.backgroundColor && ![message.backgroundColor isEqual:@""]) {
        self.containerView.backgroundColor = [CJMInAppUtils CJM_colorWithHexString:message.backgroundColor];
    } else {
        self.containerView.backgroundColor = [UIColor whiteColor];
    }
    
    self.messageType = [CJMInboxUtils inboxMessageTypeFromString:message.type];
    if ([self hasAudio] || [self hasVideo]) {
        ...
        switch (self.messageType) {
            case CJMInboxMessageTypeSimple:
                self.mediaPlayerCellType = isPortrait ? CJMMediaPlayerCellTypeTopPortrait : CJMMediaPlayerCellTypeTopLandscape;
                break;
            case CJMInboxMessageTypeCarousel:
                self.mediaPlayerCellType = CJMMediaPlayerCellTypeNone;
                break;
            ...
            case CJMInboxMessageTypeMessageIcon:
                if (message.content[0].actionHasLinks) {
                    self.mediaPlayerCellType = isPortrait ? CJMMediaPlayerCellTypeMiddlePortrait : CJMMediaPlayerCellTypeMiddleLandscape;
                } else {
                    self.mediaPlayerCellType = isPortrait ? CJMMediaPlayerCellTypeBottomPortrait : CJMMediaPlayerCellTypeBottomLandscape;
                }
                break;
            default:
                self.mediaPlayerCellType = CJMMediaPlayerCellTypeNone;
                CJMLogStaticDebug(@"unknown Inbox Message Type, defaulting to CTMediaPlayerCellTypeNone");
                break;
        }
    } else {
        self.mediaPlayerCellType = CJMMediaPlayerCellTypeNone;
...
- (void)doLayoutForMessage:(CJMInboxMessage *)message {
    // no-op in base
}
- (void)setupMessage:(CJMInboxMessage *)message {
    // no-op in base
}
...
- (BOOL)mediaIsEmpty {
    CJMInboxMessageContent *content = [self.message.content firstObject];
...
- (void)setupMediaPlayer  {
    ...
    CJMInboxMessageContent *content = self.message.content[0];
    ...
    if (content.mediaIsVideo) {
        ...
        if (content.videoPosterUrl != nil && content.videoPosterUrl.length > 0) {
            ...
            if (!self.thumbnailGenerator) {
                self.thumbnailGenerator = [[CJMVideoThumbnailGenerator alloc] init];
            }
...
#pragma mark - CJMInboxActionViewDelegate
