...
@interface CJMInAppNotification() {
...
@property (nonatomic, readwrite) CJMInAppType inAppType;
...
@property (nonatomic, readwrite) NSUInteger timeToLive;
...
@property (nonatomic, readwrite) NSArray<CJMNotificationButton *> *buttons;
...
@implementation CJMInAppNotification: NSObject
...
- (instancetype)initWithJSON:(NSDictionary *)jsonObject {
    if (self = [super init]) {
        @try {
            self.inAppType = CJMInAppTypeUnknown;
            ...
            if (self.inAppType == CJMInAppTypeUnknown) {
            ...
            if (timeToLive) {
                ...
                NSUInteger defaultTimeToLive = (long)timeToLiveEpoch;
...
- (void)configureFromJSON: (NSDictionary *)jsonObject {
    ...
    if (self.type) {
        self.inAppType = [CJMInAppUtils inAppTypeFromString:self.type];
    }
    ...
    if ([buttons isKindOfClass:[NSArray class]]) {
        ...
        for (NSDictionary *button in buttons) {
            CJMNotificationButton *ct_button = [[CJMNotificationButton alloc] initWithJSON:button];
        ...
        for (NSString *key in [buttons allKeys]) {
            CJMNotificationButton *ct_button = [[CJMNotificationButton alloc] initWithJSON:buttons[key]];
            if (ct_button && !ct_button.error) {
                [_buttons addObject:ct_button];
            }
        }
    ...
    switch (self.inAppType) {
        ...
        case CJMInAppTypeFooter:
            if  (_mediaIsGif || _mediaIsAudio || _mediaIsVideo){
                self.imageURL = nil;
                CJMLogStaticDebug(@"unable to download media, wrong media type for template");
            }
            break;
...
- (void)legacyConfigureFromJSON:(NSDictionary *)jsonObject {
    ...
    if (data) {
        ...
        if (html) {
            self.html = html;
            self.inAppType = [CJMInAppUtils inAppTypeFromString:@"custom-html"];
        }
