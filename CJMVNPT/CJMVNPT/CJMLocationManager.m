...
typedef void (^CJMLocationSuccessBlock)(CLLocationCoordinate2D);
typedef void (^CJMLocationErrorBlock)(NSString *);

@interface CJMLocationRequest : NSObject
@property (nonatomic, copy) CJMLocationSuccessBlock successBlock;
@property (nonatomic, copy) CJMLocationErrorBlock errorBlock;
@end

@implementation CJMLocationRequest
@end
...
static NSMutableArray<CJMLocationRequest *> *pendingRequests;

static NSObject *requestsLockObject;
#endif

@implementation CJMLocationManager
...
+ (void)getLocationWithSuccess:(void (^)(CLLocationCoordinate2D location))success andError:(void (^)(NSString *reason))error {
    ...
    CJMLocationRequest *request = [CJMLocationRequest new];
...
+ (void)handleSuccess:(CLLocationCoordinate2D)location {
    ...
    @synchronized (requestsLockObject) {
        for (CJMLocationRequest *request in pendingRequests) {
