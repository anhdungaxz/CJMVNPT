...
@implementation NSObject (CJMSwizzle)

+ (BOOL)CJM_swizzleMethod:(SEL)origSel_ withMethod:(SEL)altSel_ error:(NSError**)error_ {
#if OBJC_API_VERSION >= 2
...
+ (BOOL)CJM_swizzleClassMethod:(SEL)origSel_ withClassMethod:(SEL)altSel_ error:(NSError**)error_ {
    return [GetClass((id)self) CJM_swizzleMethod:origSel_ withMethod:altSel_ error:error_];
}

+ (NSInvocation*)CJM_swizzleMethod:(SEL)origSel withBlock:(id)block error:(NSError**)error {
    IMP blockIMP = imp_implementationWithBlock(block);
    NSString *blockSelectorString = [NSString stringWithFormat:@"_ct_block_%@_%p", NSStringFromSelector(origSel), block];
    ...
    [self CJM_swizzleMethod:origSel withMethod:blockSel error:nil];
    
    return origInvocation;
}

+ (NSInvocation*)CJM_swizzleClassMethod:(SEL)origSel withBlock:(id)block error:(NSError**)error {
    NSInvocation *invocation = [GetClass((id)self) CJM_swizzleMethod:origSel withBlock:block error:error];
