...
@interface CJMInAppHTMLViewController () <WKNavigationDelegate,  UIGestureRecognizerDelegate> {
    WKWebView *webView;
    CJMDismissButton *_closeButton;
    kWRSlideStatus _currentStatus;
    CJMJSInterface *_jsInterface;
}
...
@implementation CJMInAppHTMLViewController
...
- (void)loadView {
    if (self.shouldPassThroughTouches) {
        self.view = [[CJMInAppPassThroughView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
        CJMInAppPassThroughView *view = (CJMInAppPassThroughView*)self.view;
        view.delegate = self;
    } else {
        [super loadView];
    }
...
- (void)loadWebView {
    CJMLogStaticInternal(@"%@: Loading the web view", [self class]);
    ...
    CJMLogStaticInternal(@"%@: In-app notification size: %f x %f", [self class], size.width, size.height);
    ...
    CGFloat statusBarHeight = self.notification.heightPercent == 100.0 ? [[CJMInAppResources getSharedApplication] statusBarFrame].size.height : 0.0;
    ...
    switch (pos) {
        ...
        case CLTAP_INAPP_POSITION_CENTER:
            ...
            break;
        default:
            CJMLogStaticInternal(@"Unknown position %c", pos);
            return;
    }
    ...
    if (self.notification.showClose) {
        _closeButton = [CJMDismissButton new];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    CJMLogStaticInternal(@"%@: Navigation request: %@", [self class], navigationAction.request.URL);
...
- (void)showFromWindow:(BOOL)animated {
    ...
    Class windowClass = self.shouldPassThroughTouches ? CJMInAppPassThroughWindow.class : UIWindow.class;
