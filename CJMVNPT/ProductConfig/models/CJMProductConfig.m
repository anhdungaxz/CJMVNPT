...
#define CJM_DEFAULT_FETCH_CALLS 5
#define CJM_DEFAULT_FETCH_WINDOW_LENGTH 60
...
@implementation CJMProductConfig
...
- (instancetype _Nonnull)initWithConfig:(CJMInstanceConfig *_Nonnull)config privateDelegate:(id<CJMPrivateProductConfigDelegate>_Nonnull)delegate {
    self = [super init];
    if (self) {
        _config = config;
...
- (void)initProductConfigSetting {
    self.fetchConfigCalls = CJM_DEFAULT_FETCH_CALLS;
    self.fetchConfigWindowLength = CJM_DEFAULT_FETCH_WINDOW_LENGTH;
    _lastFetchTs = [CJMPreferences getIntForKey:[self storageKeyWithSuffix:kLAST_FETCH_TS_KEY] withResetValue:0];
}
...
- (void)resetProductConfigSettings {
    [CJMPreferences removeObjectForKey:[self storageKeyWithSuffix:kLAST_FETCH_TS_KEY]];
    [self initProductConfigSetting];
}
...
- (void)setFetchConfigCalls:(NSInteger)fetchConfigCalls {
    if (fetchConfigCalls <= 0) {
        _fetchConfigCalls = CJM_DEFAULT_FETCH_CALLS;
    } else {
...
- (void)setFetchConfigWindowLength:(NSInteger)fetchConfigWindowLength {
    if (fetchConfigWindowLength <= 0) {
        _fetchConfigWindowLength = CJM_DEFAULT_FETCH_WINDOW_LENGTH;
    } else {
...
- (void)persistLastFetchTs {
    [CJMPreferences putInt:self.lastFetchTs forKey:[self storageKeyWithSuffix:kLAST_FETCH_TS_KEY]];
}
...
- (void)fetchWithMinimumInterval:(NSTimeInterval)minimumInterval {
    if ([self shouldThrottleWithMinimumFetchInterval:minimumInterval])  {
        ...
        CJMLogStaticDebug(@"Fetch Error: Product Config is throttled, try again in %ds", (int)(minimumInterval - [self timeSinceLastRequest]));
    }
}
...
- (CJMConfigValue *)get:(NSString *)key {
