...
#import "CJMObjectSelector.h"
#import "CJMValueTransformers.h"
#import "CJMEditorImageCache.h"

@interface CJMABVariant ()
...
@interface CJMABVariantAction ()

@property (nonatomic, strong) NSString *name;
@property (nonatomic, strong) CJMObjectSelector *objPath;
...
+ (CJMABVariantAction *)actionWithObject:(NSDictionary *)actionObject;

- (instancetype)initWithName:(NSString *)name
                        path:(CJMObjectSelector *)path
...
+ (CJMABVariant *)variantWithData:(NSDictionary *)variantData {
    ...
    if (!variantId) {
        CJMLogStaticDebug(@"%@: Failed to construct variant, not valid variant id: %@", self, variantId);
        return nil;
    }
    NSString *experimentId = variantData[@"exp_id"];
    if (!experimentId) {
        CJMLogStaticDebug(@"%@: Failed to construct variant, not valid experiment id: %@", self, experimentId);
        return nil;
    }
    ...
    if ([actions count] <= 0 && [vars count] <= 0) {
        CJMLogStaticDebug(@"%@: Failed to construct variant: %@, must contain either actions or vars", self, variantId);
        return nil;
    }
    
    return [[CJMABVariant alloc] initWithId:variantId
...
- (void)cleanup {
    for (CJMABVariantAction *action in self.actions) {
        [action cleanup];
    }
}
...
- (void)addAction:(NSDictionary *)actionObject andApply:(BOOL)apply {
    CJMABVariantAction *action = [CJMABVariantAction actionWithObject:actionObject];
    if (action) {
...
- (BOOL)isEqualToVariant:(CJMABVariant *)variant {
...
- (BOOL)isEqual:(id)object {
    ...
    if (![object isKindOfClass:[CJMABVariant class]]) {
        return NO;
    }
    return [self isEqualToVariant:(CJMABVariant *)object];
}
...
@implementation CJMABVariantAction
...
+ (CJMABVariantAction *)actionWithObject:(NSDictionary *)actionObject {
    // Required parameters
    CJMObjectSelector *path = [CJMObjectSelector objectSelectorWithPath:actionObject[@"path"]];
    if (!path) {
        CJMLogStaticDebug(@"%@: action path is not valid: %@", self, actionObject[@"path"]);
        return nil;
    }
    
    SEL selector = NSSelectorFromString(actionObject[@"selector"]);
    if (selector == (SEL)0) {
        CJMLogStaticDebug(@"%@: action selector is not valid: %@", self, actionObject[@"selector"]);
        return nil;
    }
    
    NSArray *args = actionObject[@"args"];
    if (![args isKindOfClass:[NSArray class]]) {
        CJMLogStaticDebug(@"%@: action arguments are not valid: %@", self, args);
        return nil;
    }
    ...
    return [[CJMABVariantAction alloc] initWithName:name
                                              path:path
                                          selector:selector
                                              args:args
...
- (instancetype)initWithName:(NSString *)name path:(CJMObjectSelector *)path
    ...
    if ((self = [super init])) {
        ...
        if ([NSStringFromClass(swizzleClass) isEqualToString:@"UIViewController"]) {
            swizzleClass = nil;
            CJMLogStaticDebug(@"%@: Failed to set UIViewController as swizzle class for object path: %@, currently not supported", self, self.objPath);
        }
        if (!swizzleClass) {
            CJMLogStaticDebug(@"%@: Unable to determine swizzleClass for object path: %@ therefore not initialing the variant action", self, self.objPath);
            return nil;
        }
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super init]) {
        ...
        self.objPath = [CJMObjectSelector objectSelectorWithPath:[aDecoder decodeObjectForKey:@"path"]];
...
- (UIImage *)imageFromImagesDictionary:(NSDictionary *)imagesDictionary {
    ...
    UIImage *image = [CJMNSDictionaryFromUIImageValueTransformer imageFromDictionary:imagesDictionary];
    if (image) {
        if (!self.imageUrls) {
...
- (void)apply {
        ...
        [[self class] runSyncMainQueue:^{
            ...
            NSArray *invocations = [[self class] executeSelector:self.selector
                                                        withArgs:originalArgs
                                                          onPath:self.objPath
                                                        fromRoot:[CJMInAppResources getSharedApplication].keyWindow.rootViewController
                                                          toLeaf:nil];
    ...
    if (self.swizzle && self.swizzleClass != nil) {
        ...
        [CJMSwizzler CJM_swizzleSelector:self.swizzleSelector
...
- (void)revert {
    if (self.swizzle && self.swizzleClass != nil) {
        [CJMSwizzler CJM_unswizzleSelector:self.swizzleSelector
                                 onClass:self.swizzleClass
                                   named:self.name];
    }
...
- (void)cleanup {
    ...
    for (NSString *imageUrl in self.imageUrls) {
        [CJMEditorImageCache removeImage:imageUrl];
    }
    self.imageUrls = [NSMutableArray new];
}
...
+ (NSArray *)executeSelector:(SEL)selector withArgs:(NSArray *)args onPath:(CJMObjectSelector *)path fromRoot:(NSObject *)root toLeaf:(NSObject *)leaf {
    if (leaf) {
        ...
        if (signature != nil) {
            ...
            if (args.count >= requiredArgs) {
                ...
                @catch (NSException *exception) {
                    CJMLogStaticDebug(@"%@: Exception during invocation: %@", self, exception);
                }
                [invocations addObject:invocation];
            } else {
                CJMLogStaticDebug(@"%@: Provided args are not enough", self);
            }
        } else {
            CJMLogStaticDebug(@"%@: Method not found for %@", self, NSStringFromSelector(selector));
        }
    }
...
- (BOOL)isEqualToAction:(CJMABVariantAction *)action {
...
- (BOOL)isEqual:(id)object {
    ...
    if (![object isKindOfClass:[CJMABVariantAction class]]) {
        return NO;
    }
    return [self isEqualToAction:(CJMABVariantAction *)object];
}
