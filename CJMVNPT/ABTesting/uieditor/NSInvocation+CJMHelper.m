...
static void CJMFree(void *p){
    if (p) {
        free(p);
    }
}

static void *CJMAllocBufferForObjCType(const char *objCType) {
    void *buffer = NULL;
    ...
    if (result != 0) {
        CJMLogStaticInternal(@"Error allocating aligned memory: %s", strerror(result));
    }
...
@implementation NSInvocation (CTHelper)

- (void)CJM_setArgument:(id)argumentValue atIndex:(NSUInteger)index {
    ...
    if ([argumentValue isKindOfClass:[NSNumber class]] && strlen(argumentType) == 1) {
        ...
        switch (argumentType[0])
        {
            ...
            default:
                NSAssert(NO, @"Not supported argument type!");
        }
    ...
    else if ([argumentValue isKindOfClass:[NSValue class]])
    {
        ...
        void *buffer = CJMAllocBufferForObjCType(valueArgument.objCType);
...
- (void)CJM_setArgumentsFromArray:(NSArray *)argumentArray {
    ...
    for (id argument in argumentArray) {
        ...
        [self CJM_setArgument:argument atIndex:argumentIndex];
        ++i;
    }
}
...
- (id)CJM_returnValue {
    ...
    void *buffer = CJMAllocBufferForObjCType(objCType);
    ...
    if (strlen(objCType) == 1) {
        switch (objCType[0])
        {
            ...
            default:
                NSAssert1(NO, @"Not handling return type: %s", objCType);
                break;
        }
    } else {
        switch (objCType[0])
