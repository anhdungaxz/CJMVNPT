...
@implementation CJMNSDictionaryFromUIImageValueTransformer
...
+ (UIImage *)imageFromDictionary:(NSDictionary *)imagesDictionary {
    ...
    if (imageDictionary[@"url"]) {
        ...
        image = [CJMEditorImageCache getImage:url withScale:scale.floatValue andSize:size];
        if (!image) {
            return nil;
        }
    ...
    if (image && imagesDictionary[@"capInsets"]) {
        NSValueTransformer *insetsTransformer = [NSValueTransformer valueTransformerForName:NSStringFromClass([CJMNSDictionaryFromUIEdgeInsetsValueTransformer class])];
...
- (id)transformedValue:(id)value {
    ...
    if ([value isKindOfClass:[UIImage class]]) {
        ...
        NSValueTransformer *sizeTransformer = [NSValueTransformer valueTransformerForName:NSStringFromClass([CJMNSDictionaryFromCGSizeValueTransformer class])];
        NSValueTransformer *insetsTransformer = [NSValueTransformer valueTransformerForName:NSStringFromClass([CJMNSDictionaryFromUIEdgeInsetsValueTransformer class])];
