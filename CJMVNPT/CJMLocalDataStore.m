...
@implementation CJMLocalDataStore
...
- (BOOL)inBackgroundQueue {
    CJMLocalDataStore *currentQueue = (__bridge id) dispatch_get_specific(kProfileBackgroundQueueKey);
...
- (void)addDataSyncFlag:(NSMutableDictionary *)event {
    ...
    @try {
        ...
        if (now > last + expiry) {
            event[@"dsync"] = @YES;
            CJMLogInternal(self.config.logLevel, @"%@: Local data cache needs to be updated", self);
        } else {
            CJMLogInternal(self.config.logLevel, @"%@: Local data cache doesn't need to be updated", self);
        }
    } @catch (NSException *e) {
        CJMLogInternal(self.config.logLevel, @"%@: Failed to add the data sync flag: %@",self, e.debugDescription);
    }
}
...
- (NSDictionary *)getStoredEvents {
    NSDictionary *events = [CJMPreferences getObjectForKey:[self storageKeyWithSuffix:kWR_KEY_EVENTS]];
    if (self.config.isDefaultInstance) {
        if (!events) {
            events = [CJMPreferences getObjectForKey:kWR_KEY_EVENTS];
            if (events) {
                [CJMPreferences removeObjectForKey:kWR_KEY_EVENTS];
...
- (void)setStoredEvents:(NSDictionary *)store {
    [CJMPreferences putObject:store forKey:[self storageKeyWithSuffix:kWR_KEY_EVENTS]];
}

- (void)clearStoredEvents {
    [CJMPreferences removeObjectForKey:[self storageKeyWithSuffix:kWR_KEY_EVENTS]];
}
    ...
    @try {
        ...
        for (NSString *eventName in [events keyEnumerator]) {
            ...
            if (!ed) {
                ...
                CJMLogInternal(self.config.logLevel, @"%@: Upstream event sync - Event \"%@\" not available locally. Trusting upstream", self, eventName);
                evDidUpdate = YES;
            } else {
                ...
                if (upstreamOccurrences.intValue > localOccurrences.intValue) {
                    store[eventName] = upstreamEvent;
                    CJMLogInternal(self.config.logLevel, @"%@: Upstream event sync - Event \"%@\" has updated details. Trusting upstream", self, eventName);
                    evDidUpdate = YES;
                } else {
                    CJMLogInternal(self.config.logLevel, @"%@: Upstream event sync - Event \"%@\" has been rejected from upstream", self, eventName);
                }
            }
            if (evDidUpdate) {
                ...
                @catch (NSException *e) {
                    CJMLogInternal(self.config.logLevel, @"Failed to set event changes for event: %@ for reason: %@", eventName, [e reason]);
                    continue;
                }
...
- (NSDictionary *)syncProfile:(NSDictionary *)remoteProfile {
    ...
    @try {
        ...
        for (NSString *key in keys) {
            @try {
                ...
                if ([self shouldPreferLocalProfileUpdateForKey:key forDate:now]) {
                    CJMLogInternal(self.config.logLevel, @"%@: Rejecting update for key:%@ in favor of locally updated value", self, key);
                    continue;
                }
                ...
                if ([CJMLocalDataStore profileValueIsEmpty:remoteValue]) {
                    remoteValue = nil;
                }
        ...
        CJMLogDebug(self.config.logLevel, @"Failed to persist profile: %@", [e reason]);
        return @{};
    }
}
    ...
    @try {
        ...
        if (somethingGotUpdated) {
            @try {
                ...
                CJMLogInternal(self.config.logLevel, @"%@: Failed to process profile updates: %@",self, e.debugDescription);
                return nil;
            }
        }
    } @catch (NSException *e) {
        CJMLogInternal(self.config.logLevel, @"%@: Failed to process data sync from upstream: %@", self, e.debugDescription);
        return nil;
    }
}
...
- (CJMEventDetail *)getEventDetail:(NSString *)event {
    @try {
        ...
        CJMEventDetail *ed = [[CJMEventDetail alloc] init];
...
- (NSDictionary *)getEventHistory {
    @try {
        ...
        for (NSString *eventName in [s keyEnumerator]) {
            ...
            CJMEventDetail *ev = [[CJMEventDetail alloc] init];
...
- (void)_setProfileValue:(id)value forKey:(NSString *)key fromUpstream:(BOOL)fromUpstream {
    ...
    @catch (NSException *exception) {
        CJMLogInternal(self.config.logLevel, @"%@: Exception setting profile field %@ in session cache for value %@", self, key, value);
    }
}
...
- (void)_removeProfileValueForKey:(NSString *)key fromUpstream:(BOOL)fromUpstream {
    ...
    @catch (NSException *exception) {
        CJMLogInternal(self.config.logLevel, @"%@: Exception removing profile field %@ in session cache", self, key);
    }
...
- (NSMutableDictionary *)_inflateLocalProfile {
    NSMutableDictionary *_profile = (NSMutableDictionary *)[CJMPreferences unarchiveFromFile:[self profileFileName] removeFile:NO];
    if (!_profile) {
...
- (void)_persistLocalProfileAsync {
    [self runOnBackgroundQueue:^{
        ...
        [CJMPreferences archiveObject:_profile forFileName:[self profileFileName]];
    }];
}
...
- (void)setLocalCacheExpiryInterval:(int)interval {
    [CJMPreferences putInt:interval forKey:[self storageKeyWithSuffix:kLocalCacheExpiry]];
}

- (long)getLocalCacheExpiryIntervalWithResetValue:(int)resetValue {
    if (self.config.isDefaultInstance) {
        return  [CJMPreferences getIntForKey:[self storageKeyWithSuffix:kLocalCacheExpiry] withResetValue:[CJMPreferences getIntForKey:kLocalCacheExpiry withResetValue:resetValue]];
    } else {
        return [CJMPreferences getIntForKey:[self storageKeyWithSuffix:kLocalCacheExpiry] withResetValue:resetValue];
    }
}
...
- (long)getLastDataSyncTimeWithResetValue:(long)resetValue {
    if (self.config.isDefaultInstance) {
        return [CJMPreferences getIntForKey:[self storageKeyWithSuffix:kLocalCacheLastSync] withResetValue:[CJMPreferences getIntForKey:kLocalCacheLastSync withResetValue:resetValue]];
    } else {
        return [CJMPreferences getIntForKey:[self storageKeyWithSuffix:kLocalCacheLastSync] withResetValue:resetValue];
    }
}

- (void)setLastDataSyncTime:(long)time {
    [CJMPreferences putInt:time forKey:[self storageKeyWithSuffix:kLocalCacheLastSync]];
}
