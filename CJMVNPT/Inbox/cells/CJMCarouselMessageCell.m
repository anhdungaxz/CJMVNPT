...
#import "CJMCarouselImageView.h"

@implementation CJMCarouselMessageCell
...
- (void)populateLandscapeViews {
    ...
    NSUInteger index = 0;
    for (CJMInboxMessageContent *content in (self.message.content)) {
        CJMCarouselImageView *carouselItemView;
        if (carouselItemView == nil){
            carouselItemView  = [[[CJMInAppUtils bundle] loadNibNamed: NSStringFromClass([CJMCarouselImageView class]) owner:nil options:nil] lastObject];
            ...
            carouselItemView.titleLabel.textColor = content.titleColor ? [CJMInAppUtils CJM_colorWithHexString:content.titleColor] : [CJMInAppUtils CJM_colorWithHexString:@"#000000"];
            carouselItemView.bodyLabel.text = content.message;
            carouselItemView.bodyLabel.textColor = content.messageColor ? [CJMInAppUtils CJM_colorWithHexString:content.messageColor] :  [CJMInAppUtils CJM_colorWithHexString:@"#7E7E7E"];
        }
...
- (void)populateItemViews {
    ...
    for (CJMInboxMessageContent *content in (self.message.content)) {
        ...
        CJMCarouselImageView *itemView;
        if (itemView == nil){
            CGRect frame = self.carouselView.bounds;
            frame.size.height =  frame.size.height;
            itemView = [[CJMCarouselImageView alloc] initWithFrame:self.carouselView.bounds
...
- (void)setupMessage:(CJMInboxMessage *)message {
    ...
    if ([self deviceOrientationIsLandscape]) {
        ...
        if (@available(iOS 11.0, *)) {
            UIWindow *window = [CJMInAppResources getSharedApplication].keyWindow;
        ...
        captionHeight = [CJMCarouselImageView captionHeight];
...
- (void)configureSwipeViewWithHeightAdjustment:(CGFloat)adjustment {
    self.swipeView = [[CJMSwipeView alloc] init];
...
- (void)handleItemViewTapGesture:(UITapGestureRecognizer *)sender {
    CJMCarouselImageView *itemView = (CJMCarouselImageView*)sender.view;
