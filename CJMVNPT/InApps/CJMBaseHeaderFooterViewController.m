...
@interface CJMBaseHeaderFooterViewController () <UIGestureRecognizerDelegate> {
...
@implementation CJMBaseHeaderFooterViewController

- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil notification:(CJMInAppNotification *)notification {
...
- (void)layoutNotification {
    ...
    self.containerView.backgroundColor = [CJMInAppUtils CJM_colorWithHexString:self.notification.backgroundColor];
...
- (void)setUpContent {
    ...
    if (self.notification.title) {
        ...
        self.titleLabel.textColor = [CJMInAppUtils CJM_colorWithHexString:self.notification.titleColor];
    ...
    if (self.notification.message) {
        ...
        self.bodyLabel.textColor = [CJMInAppUtils CJM_colorWithHexString:self.notification.messageColor];
    ...
    if (@available(iOS 13, *)) {
        ...
        for (UIScene *scene in connectedScenes) {
            if (scene.activationState == UISceneActivationStateForegroundActive && [scene isKindOfClass:[UIWindowScene class]]) {
                ...
                self.window = [[CJMInAppPassThroughWindow alloc] initWithFrame:
                               windowScene.coordinateSpace.bounds];
                self.window.windowScene = windowScene;
            }
        }
    } else {
        self.window = [[CJMInAppPassThroughWindow alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
    }
    self.window.alpha = 0;
